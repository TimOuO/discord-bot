...
import { RPGService, FISH_COOLDOWN_MS } from "../../services/rpgService";
...
function buildFishRetryButton(ownerId: string): ButtonBuilder {
  return new ButtonBuilder()
    ...
    .setLabel(`再釣一次（冷卻 ${formatCooldown(FISH_COOLDOWN_MS)}）`)
    .setEmoji("🎣")
...
async function runFishAndBuildReply(userId: string, username: string, avatarURL: string) {
  ...
  const lines = result.items.map((entry) => {
    ...
    return `▷ 釣到了一隻 \`${entry.item.name}\`（${rarityLabel}）　目前 ${chip(entry.quantity)} 隻`;
  });

  const embed = new EmbedBuilder()
    .setAuthor({ name: username, iconURL: avatarURL })
    .setTitle(result.items.length > 1 ? "🎣 釣魚成功！（雙倍收穫）" : "🎣 釣魚成功！")
  ...
  return {
    embeds: [embed],
    components: [buildFishActionRow(userId, primary.item.name, unitPrice, primary.quantity)],
  };
}
...
export async function handleFishCommand(interaction: ChatInputCommandInteraction) {
  try {
    ...
    const message = describeCommandError("RPG Fish 命令錯誤", error);
...
export async function handleFishRetryButton(interaction: ButtonInteraction) {
  ...
  try {
    ...
    const message = describeCommandError("釣魚按鈕錯誤", error);
