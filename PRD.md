...
| 星火匠神 | 強化 | 失敗退級門檻從 +6 推到 +8 | 1.3× |
| 荒野獵者 | 生產 | 釣魚採集擲兩次獨立骰子，且絕對不會空手 | 2.0× |

**三個被數字改掉的初版設計**：

- **「菁英怪出現率 20%→30%」跟「採集冷卻減半」被否決**。玩家的評語是「超廢」，而他是對的：那兩個改的是**速率**不是規則，冷卻減半更糟——它不是讓你變強，是讓你多打字。
- **星火匠神原本要保護 +6~+8**，模擬算出 epic 成本直接減半（81→40），比其他職業強太多。改成只保護 +6、+7 是 −24%，而且 **legendary 完全不受影響**（12.9→12.9），因為它花在 +9/+10、那兩級照樣退級。這個被動天生自我限制，不會廢掉第 19 節的材料沖水槽。
