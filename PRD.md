...
## 15. 平衡模擬腳本 + 戰鬥純函式抽離（grilling 結論，2026-09-04）

**背景**：過去每次調戰鬥數值（百分比減傷、地下城 boss 倍率、死亡螺旋保底、稀有度權重）都是臨時寫一支腳本跑完就刪，下次要調又得重寫，而且沒有留下可比較的基準。這次把它固定下來。

**`src/services/combat.ts`（新）**：把不碰資料庫的純規則從 `rpgService` 抽出來——傷害公式、敵人生成、單場回合模擬、勝利回血、落敗保底、升級結算、經驗門檻，以及所有可調參數常數。抽出來的直接原因是 `dbService` 在模組載入時就 `new PrismaClient()` 並讀 `DATABASE_URL`，模擬腳本只是要算數學卻得先有資料庫才能跑；順帶好處是所有可調數值集中在同一個檔案，不用在近千行的 service 裡東翻西找。原本內嵌在 `battle()` 裡的敵人等級擲骰也一併抽成 `rollEnemyLevel()`，讓模擬用的是「真正在跑的規則」而不是另外照抄一份（照抄的版本一定會漂移）。

**`npm run sim:balance`**：印出三張表——單場滿血勝率、連續場次穩態勝率（血量延續、不喝藥水）、升級節奏。等級軸 1/5/6/10/20/40/80，裝備軸 無裝備/新手套/中期套/神話級（數值寫死在腳本裡並註解對應的實際裝備，保持腳本不依賴資料庫）。腳本內部把 `Math.random` 暫時換成固定種子的 mulberry32，**每次跑出同一張表**，改公式時才分得出是真的有差還是雜訊——這件事只有在前一批把全專案亂數統一到 `Math.random` 之後才做得到（`crypto.randomInt` 沒辦法這樣換）。

**模擬結果與三個結構性問題**（都獨立重現了外部平衡分析的數字，誤差 2~3 個百分點以內）：

1. **Lv6 懸崖**：`lossHealthFloor` 在 Lv5→Lv6 是硬切（80% 掉到 30%）。連續場次穩態勝率：Lv5 沒裝備 48%、新手套 61%；**Lv6 直接掉到 3% / 4%**。死亡螺旋沒有被消除，只是從 Lv1 搬到了 Lv6。
2. **裝備是固定值、敵人是線性成長**：玩家與敵人的攻防成長完全相同（Lv L 都是 攻 8+2L、防 4+L），差距只能靠裝備拉開；但裝備是 +N 固定值，等級越高佔比越小。中期套在 Lv20 掉到 13%、神話級在 Lv80 掉到 10%，而神話級已經是目前的天花板。
3. **升級節奏是常數**：每級所需場次收斂到約 20 場（實測 Lv1 8.3 場 → Lv80 19.5 場），30 秒冷卻等於固定 10 分鐘一級，沒有上限，排行榜實質上是比在線時間。

**一個被誤解的數字**：外部分析（跟我最初的複述）都以為敵人平均比玩家高半級。實際上 `battle()` 的敵人等級是 `max(1, 玩家等級 - 1 + 偏移)`，Lv6+ 的偏移是 {-1,0,1,2}，**平均落在玩家等級 -0.5**，滿血單場對打玩家反而略佔優勢。這讓結論更明確：問題純粹出在血量延續，不是單場公平性。另外單場勝率表的數值會卡在 25% 倍數附近，那不是雜訊——敵人等級只有 4 種等機率結果，每一種通常是決定性的輸或贏，所以是階梯函數。

**`src/services/combat.test.ts`（新）**：少量寬鬆的回歸斷言進現有測試套件（純數學模擬很快，整體測試時間只從 6.5 秒變 7 秒，不會拖慢部署 gate）。其中「Lv6 沒裝備連續作戰勝率低於兩成」是**刻意把現況釘住**——等哪天修好 Lv6 懸崖，這條會變紅燈提醒更新期望值，而不是讓問題悄悄消失。
